...
struct GCDDataManager: DataManager {
    ...
    let archiveURL: URL
    let syncQueue = DispatchQueue(label: "com.achanov", qos: .userInitiated) // делаем очередь последовательной, чтобы избежать race condition, т.к. если попробовать загрузить профиль во время сохранения, может загрузиться старый профиль
