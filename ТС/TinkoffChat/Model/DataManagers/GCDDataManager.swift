...
struct GCDDataManager {
    ...
    let archiveURL: URL
    let syncQueue = DispatchQueue(label: "com.chanov", qos: .userInitiated) // делаем очередь последовательной, чтобы избежать race condition, т.к. если попробовать загрузить профиль во время сохранения, может загрузиться старый профиль
